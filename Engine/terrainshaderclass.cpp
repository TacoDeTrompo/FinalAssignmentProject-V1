...
bool TerrainShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Engine/terrain.vs", L"../Engine/terrain.hlsl");
	if(!result)
	{
		return false;
	}
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
								...
								ID3D11ShaderResourceView** texture, int countText)
{
	bool result;

	for (int i = 0; i < countText; i++) {
		// Set the shader parameters that it will use for rendering.
		result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, ambientColor, diffuseColor, lightDirection, texture, countText);
		if (!result)
		{
			return false;
		}
