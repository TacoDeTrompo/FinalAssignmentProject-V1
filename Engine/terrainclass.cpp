...
bool TerrainClass::Initialize(ID3D11Device* device, char* heightMapFilename, WCHAR** textureFilename, int countText)
{
	bool result;
	...
	// Load the texture.
	result = LoadTexture(device, textureFilename, countText);
	if(!result)
	{
		return false;
	}
...
void TerrainClass::Shutdown(int CountText)
{
	// Release the texture.
	ReleaseTexture(countText);
...
ID3D11ShaderResourceView** TerrainClass::GetTexture(int countText)
{
	ID3D11ShaderResourceView** texture = new ID3D11ShaderResourceView*[countText];
	for (int i = 0; i < countText; i++)
		texture[i] = m_Texture[i]->GetTexture();
	return texture;
}
...
bool TerrainClass::LoadTexture(ID3D11Device* device, WCHAR** filename, int countText)
{
	bool result;

	// Create the texture object.
	m_Texture = new TextureClass*[countText];
	if (!m_Texture)
	{
		return false;
	}
...
void TerrainClass::ReleaseTexture(int countText)
{
...
bool TerrainClass::GetHeightAtPosition(float positionX, float positionZ, float& height)
{
	...
	if (positionX < 0.0f || positionZ < 0.0f || positionX >= m_terrainWidth - 1 || positionZ >= m_terrainHeight - 1) 
		return 0.0f;
	...
	height = (BL * (1.0 - fX) + BR * fX) * (1.0 - fZ) + (TL * (1.0 - fX) + TR * fX) * (fZ) + 1.0f;
}
...
bool TerrainClass::InitializeBuffers(ID3D11Device* device)
{
	...
	float tu, tv;
	float tu2Left, tu2Right, tv2Top, tv2Bottom, incrementSize, incrementSize1;
	...
	incrementSize = 1.0f / m_terrainHeight;
	incrementSize1 = 1.0f / m_terrainWidth;

	// Initialize the texture increments.
	tu2Left = 0.0f;
	tu2Right = incrementSize1;
	tv2Bottom = 1.0f;
	...
	for(j=0; j<(m_terrainHeight-1); j++)
	{
		for(i=0; i<(m_terrainWidth-1); i++)
		{
			...
			vertices[index].texture2 = D3DXVECTOR2(tu2Left, tv2Top);;
			...
			vertices[index].texture2 = D3DXVECTOR2(tu2Right, tv2Top);;
			indices[index] = index;
			index++;
			...
			vertices[index].texture2 = D3DXVECTOR2(tu2Left, tv2Bottom);
			indices[index] = index;
			index++;
			...
			vertices[index].texture2 = D3DXVECTOR2(tu2Right, tv2Bottom);
			indices[index] = index;
			index++;
